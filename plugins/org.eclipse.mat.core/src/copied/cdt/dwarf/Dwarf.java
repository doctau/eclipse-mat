...
public class Dwarf {
	...
	void parseDebugInfoEntry(IDebugEntryRequestor requestor, ByteBuffer in, Map<Long, AbbreviationEntry> abbrevs, CompilationUnitHeader header)
		...
		while (in.remaining() > 0) {
			...
			if (entry != null) {
				...
				processDebugInfoEntry(requestor, entry, list, header);
			}
		}
	}
	...
	void processDebugInfoEntry(IDebugEntryRequestor requestor, AbbreviationEntry entry, List<AttributeValue> list, CompilationUnitHeader header) {
		int len = list.size();
		...
		switch (tag) {
			...
			case DwarfConstants.DW_TAG_variable :
                processVariable(requestor, list, header);
				break;
    ...
    void processVariable(IDebugEntryRequestor requestor, List<AttributeValue> list, CompilationUnitHeader header) {
        String varName = null;
        DebugType type = new DebugUnknownType("UNSET");
        DebugVariableKind kind = DebugVariableKind.UNKNOWN;
        long address = -1;
        for (int i = 0; i < list.size(); i++) {
            AttributeValue av = list.get(i);
            try {
                ...
                switch(name) {
                    case DwarfConstants.DW_AT_name:
                        //form=DW_FORM_strp
                        varName = (String)av.value;
                        break;

                    case DwarfConstants.DW_AT_location:
                        if (av.attribute.form == DwarfConstants.DW_FORM_addr) {
                            address = -4;
                            av.value.toString();
                        } else if (av.attribute.form == DwarfConstants.DW_FORM_exprloc) { 
                            Operation op = parseDwarfOp(header, (byte[]) av.value);
                            switch (op.tag) {
                                case DwarfConstants.DW_OP_addr:
                                    address = (Long)op.value;
                                    break;
                                default:
                                    // should not happen
                                    address = -2;
                    ...
                    case DwarfConstants.DW_AT_type:
                        // DW_FORM_ref4
                        type = new DebugUnknownType(av.value.toString());
                        break;
                    ...
                    case DwarfConstants.DW_AT_decl_line:
                        //ignore
                        break;
                }
            } catch (ClassCastException e) {
        ...
        requestor.acceptVariable(varName, type, kind, address);
    }

	private Operation parseDwarfOp(CompilationUnitHeader header, byte[] value) {
	    Operation op = new Operation();
	    op.tag = value[0];

	    // delay until later?
        byte[] rest = new byte[value.length-1];
        System.arraycopy(value, 1, rest, 0, value.length - 1);
        switch (op.tag) {
            case DwarfConstants.DW_OP_addr:
                // pointer
                if (header.addressSize == 8)
                    op.value = read_8_bytes(rest);
                else
                    op.value = (long)read_4_bytes(rest);
                break;
            default:
                op.value = rest;
        }
